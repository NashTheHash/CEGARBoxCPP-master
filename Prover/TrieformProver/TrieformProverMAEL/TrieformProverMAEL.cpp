#include "TrieformProverMAEL.h"

shared_ptr<Cache> TrieformProverMAEL::persistentCache = make_shared<PrefixCache>("P");

unsigned int TrieformProverMAEL::assumptionsSize = 0;
GlobalSolutionMemo TrieformProverMAEL::globalMemo = GlobalSolutionMemo();
unordered_map<string, unsigned int> TrieformProverMAEL::idMap =
    unordered_map<string, unsigned int>();

shared_ptr<Trieform>
TrieformFactory::makeTrieMAEL(const shared_ptr<Formula> &formula,
                            shared_ptr<Trieform> trieParent) {
  shared_ptr<Trieform> trie = shared_ptr<Trieform>(new TrieformProverMAEL());
...
shared_ptr<Trieform>
TrieformFactory::makeTrieMAEL(const vector<int> &newModality,
                            shared_ptr<Trieform> trieParent) {
  shared_ptr<Trieform> trie = shared_ptr<Trieform>(new TrieformProverMAEL());
...
TrieformProverMAEL::TrieformProverMAEL() {}
TrieformProverMAEL::~TrieformProverMAEL() {}

shared_ptr<Trieform>
TrieformProverMAEL::create(const shared_ptr<Formula> &formula) {
  return TrieformFactory::makeTrieMAEL(formula, shared_from_this());
}
shared_ptr<Trieform>
TrieformProverMAEL::create(const shared_ptr<Formula> &formula,
                         const vector<int> &newModality) {
  return TrieformFactory::makeTrieMAEL(formula, newModality, shared_from_this());
}
shared_ptr<Trieform> TrieformProverMAEL::create(const vector<int> &newModality) {
  return TrieformFactory::makeTrieMAEL(newModality, shared_from_this());
}

shared_ptr<Bitset>
TrieformProverMAEL::convertAssumptionsToBitset(literal_set literals) {
...
void TrieformProverMAEL::updateSolutionMemo(const shared_ptr<Bitset> &assumptions,
                                          Solution solution) {
...
bool TrieformProverMAEL::isInHistory(vector<shared_ptr<Bitset>> history,
                                   shared_ptr<Bitset> bitset) {
...
void TrieformProverMAEL::reflexiveHandleBoxClauses() {
  ...
  for (auto modalSubtrie : subtrieMap) {
    dynamic_cast<TrieformProverMAEL *>(modalSubtrie.second.get())
        ->reflexiveHandleBoxClauses();
  }
}

void TrieformProverMAEL::reflexivepropagateLevels() {
  for (auto modalSubtrie : subtrieMap) {
    dynamic_cast<TrieformProverMAEL *>(modalSubtrie.second.get())
...
void TrieformProverMAEL::pruneTrie() {
  for (auto modalSubtrie : subtrieMap) {
    if (modalSubtrie.second->hasSubtrie(modalSubtrie.first)) {
      modalSubtrie.second->removeSubtrie(modalSubtrie.first);
    }
    dynamic_cast<TrieformProverMAEL *>(modalSubtrie.second.get())->pruneTrie();
  }
}

void TrieformProverMAEL::makePersistence() {
  for (auto modalSubtrie : subtrieMap) {
    dynamic_cast<TrieformProverMAEL *>(modalSubtrie.second.get())
        ->makePersistence();
  }
...
void TrieformProverMAEL::propagateSymmetricBoxes() {
  for (auto modalitySubtrie : subtrieMap) {
    dynamic_cast<TrieformProverMAEL *>(modalitySubtrie.second.get())
...
void TrieformProverMAEL::preprocess() {
...
void TrieformProverMAEL::prepareSAT(name_set extra) {
...
Solution TrieformProverMAEL::prove(vector<shared_ptr<Bitset>> history,
  ...
  for (auto modalityDiamonds : triggeredDiamonds) {
    ...
    while (!diamondPriority.empty()) {
      ...
      if (getLevel() == 1) {
        ...
        childSolution = dynamic_cast<TrieformProverMAEL *>(
...
Solution TrieformProverMAEL::prove(literal_set assumptions = literal_set()) {
