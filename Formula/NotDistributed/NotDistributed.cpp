...
vector<int> NotDistributed::getModalityList() const { 
...
int NotDistributed::getPower() const{
    return power_;
}


shared_ptr<Formula> NotDistributed::getSubformula() const{
    return subformula_;
}


void NotDistributed::incrementPower() {
    power_++;
}
...
FormulaType NotDistributed::getType() const {
    return FNotDistributed;
}
...
shared_ptr<Formula> NotDistributed::create(vector<int> modality, int power, const shared_ptr<Formula> &subformula) {
...
shared_ptr<Formula> NotDistributed::create(vector<vector<int>> modality, const shared_ptr<Formula> &subformula) {

    if (modality.size() == 0) return subformula;

    shared_ptr<Formula> formula = NotDistributed::create(modality[modality.size() - 1], 1, subformula);
...
bool NotDistributed::operator==(const Formula &other) const {
    ...
    return modalityListString_ == otherNotDistributed->modalityListString_ &&
        power_ == otherNotDistributed->power_ &&
