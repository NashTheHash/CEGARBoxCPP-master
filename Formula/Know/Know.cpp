...
int Know::getModality() const {
    return modality_;
}


int Know::getPower() const {
    return power_;
}


shared_ptr<Formula> Know::getSubformula() const {
    return subformula_;
}


void Know::incrementPower() {
    power_++;
}
...
FormulaType Know::getType() const {
    return FKnow;
}
...
shared_ptr<Formula> Know::create(int modality, int power, const shared_ptr<Formula> &subformula) {
...
shared_ptr<Formula> Know::create(vector<int> modality, const shared_ptr<Formula> &subformula) {

    if (modality.size() == 0) return subformula;
    shared_ptr<Formula> formula = Know::create(modality[modality.size() - 1], 1, subformula);
...
bool Know::operator==(const Formula &other) const {
    ...
    return modality_ == otherKnow->modality_ && power_ == otherKnow->power_ && *subformula_ == *(otherKnow->subformula_);
}
